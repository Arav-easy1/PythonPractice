# index(원소): 리스트 내 특정한 원소의 인덱스를 찾기
    # list = ['이지원', '김나영', '형연지', '장묘주', '박수정', '강다원', '박소희']
    # print(list.index('박수정'))    # 4 출력됨. 존재하지 않으면 오류남
# reverse(): 리스트의 원소를 뒤집기
    # list = [1, 2, 3]
    # list.reverse()
    # print(list)
    # # 슬라이싱도 이용가능!!!!!!!!!!!!!!
    # print(list[::-1])       # 슬라이싱은 일회용임 담아줘야됨.
# sum(리스트 자료형): 리스트의 모든 원소의 합
    # list = [1, 2, 0.75]     # 문자랑 섞으면 오류남
    # print(sum(list))
# range(시작, 끝): 특정 범위를 지정
# list(특정 범위): 특정 범위의 원소를 가지는 리스트를 반환
    # my_range = range(5, 10)
    # list = list(my_range)
    # print(list)
# all() / any(): 리스트의 모든 원소가 참인지 판별 / 하나라도 참인지 판별
    # list = [True, False, True]
    # print(all(list))
    # print(any(list))
# enumerate(): 리스트에서 인덱스와 원소를 함께 추출
    # my_list = ['이지원', '김나영', '형연지']
    # result = list(enumerate(my_list))
    # print(result)
    # for i, k in enumerate(my_list):
    #     print("인덱스: ", i, "원소: ", k)
# sort(): 리스트의 원소를 정렬
    # my_list = ['이지원', '김나영', '형연지']
    # my_list.sort()
    # print(my_list)
# count(): 리스트에서 특정한 원소의 개수를 추출
    # my_list = ['이지원', '김나영', '김나영', '김나영', '형연지']
    # print(my_list.count('김나영'))     # 나영이 3번나와서 3출력
# del: 리스트의 특정 원소를 제거
    # my_list = ['123', '345', '678', '345', '678', '345', '678', '345', '678', '345', '678']
    # del my_list[1]
    # print(my_list)
    # del my_list[1:5]
    # print(my_list)
# insert(): 리스트에 특정 원소를 삽입
    # my_list = ['123', '345', '678', '345', '678', '345', '678', '345', '678', '345', '678']
    # my_list.insert(3, '-1')
    # print(my_list)
# append(): 리스트의 가장 마지막 원소로서 원소를 삽입
    # my_list = ['123', '345', '678', '345', '678', '345', '678', '345', '678', '345', '678']
    # my_list.append('-1')
    # print(my_list)